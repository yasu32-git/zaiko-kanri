 ...
 *
 * 在庫0は「リストから除外」中でも最優先で「必須」にして自動でリスト入りさせる
 * （14日間の除外はあくまで通常の補充提案を抑制するものであり、完全に切らした場合の
 * 緊急シグナルまでは止めない）。ただし手動で緊急度を「不要」にしている品目は、
 * その明示的な指定を尊重して在庫0でも変更しない。
 *
 * それ以外の品目は、「リストから除外」した品目なら除外期限（除外した日から14日後）を
 * 過ぎるまで自動では一切リストに追加しない。
 *
 * さらにそれ以外は、目標在庫数が設定されていれば在庫数によって以下の状態に分かれる。
...
function applyAutoListRules(item) {
  if (item.stock <= 0 && item.urgency !== '不要') {
